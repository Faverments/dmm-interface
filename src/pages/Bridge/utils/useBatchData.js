...
function getBatchResult({ chainId, rpc, calls, provider }) {
  return new Promise((resolve, reject) => {
    ...
    Promise.race([timeoutWeb3(), useMethods({ chainId, rpc, calls, provider })])
      .then(res => {
        if (res === TIMEOUT) {
          reject(res)
        } else {
...
export async function useBatchData({ chainId, calls, provider }) {
  ...
  try {
    ...
    if (
      error.toString().indexOf('Error: Returned error: execution reverted: Multicall aggregate: call failed') !== -1
    ) {
