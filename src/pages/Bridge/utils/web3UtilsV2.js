...
function getBatchWeb3Result(rpc, list) {
  return new Promise((resolve, reject) => {
    Promise.race([timeoutWeb3(), getBatchWeb3Data(rpc, list)])
      .then(res => {
        if (res === TIMEOUT) {
          reject(res)
        } else {
...
function getWeb3Result(rpc, property, name, params) {
  return new Promise((resolve, reject) => {
    Promise.race([timeoutWeb3(), getWeb3Data(rpc, property, name, params)])
      .then(res => {
        if (res === TIMEOUT) {
          reject(res)
        } else {
...
export { getWeb3, useWeb3, useBatchWeb3, getContract, getErcContract }
