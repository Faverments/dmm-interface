...
import useGetTrendingHistoryData, {
  TrendingHistoryData,
  TrendingHistoryResponse,
} from 'pages/DiscoverPro/hooks/useGetTrendingHistoryData'
import { HeaderDetails } from '../..'
import useFilterTrendingHistoryData from 'pages/DiscoverPro/hooks/useFilterTrendingHistoryData'
...
const TrendingLayout = ({
  ...
  trendingHistoryResponse: TrendingHistoryResponse
  setTrendingHistoryResponse: React.Dispatch<React.SetStateAction<TrendingHistoryResponse>>
  trendingHistoryData: TrendingHistoryData | null
  setTrendingHistoryData: React.Dispatch<React.SetStateAction<TrendingHistoryData | null>>
  ...
  }, [isLoadingTrendingHistoryRes])

  const filterTrendingHistoryData = useFilterTrendingHistoryData(
    filter.isShowTokensBeforeChange ? trendingHistoryRes?.current_data_before_change?.data : trendingHistoryData?.data,
    filter,
  )

  const trendingSoonTokens = useMemo(() => filterTrendingHistoryData?.tokens ?? [], [filterTrendingHistoryData])

  const sortedPaginatedTrendingSoonTokens = useMemo(() => {
    return trendingSoonTokens.slice((currentPage - 1) * TRENDING_ITEM_PER_PAGE, currentPage * TRENDING_ITEM_PER_PAGE)
  }, [trendingSoonTokens, currentPage])
  ...
  const MobileLayout = () => (
    <Box overflow="hidden">
      {sortedPaginatedTrendingSoonTokens.map(tokenData => (
      ...
      <Pagination
        ...
        totalCount={filterTrendingHistoryData?.total_number_tokens ?? 1}
      />
      <MobileChartModal
