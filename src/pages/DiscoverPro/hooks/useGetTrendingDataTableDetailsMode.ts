...
export default function useGetTrendingDataTableDetailsMode(
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const url = `${process.env.REACT_APP_TRUESIGHT_API}/api/v1/trending?timeframe=${timeframe}&page_number=${
          filter.isShowTrueSightOnly ? 0 : 0
        }&page_size=${filter.isShowTrueSightOnly ? 9999 : maxItems}&search_token_id=${filter.selectedTokenData
          ?.token_id ?? ''}&search_token_tag=${filter.selectedTag ?? ''}`
        setError(undefined)
        setIsLoading(true)
    ...
    fetchData()
  }, [currentPage, filter, itemPerPage, maxItems])
